...
def calculate_rmse_per_time_step(persistent_data, predict_data, label_data):
    ...
    numpoints, t_span, time_steps = persistent_data.shape
...
def plot_process(label_all_save,predict_all_save,persistent_data, predict_data, label_data,t_start,t_gap):

    error = np.abs(predict_all_save - label_all_save)
    ...
    time_points = np.arange(t_start, t_start + t_gap * label_all_save.shape[1],20/t_gap)

    y_labels = np.arange(0, label_all_save.shape[0], 20)
    ...
    cmap = LinearSegmentedColormap.from_list("custom_gist_rainbow", colors)
    
    plt.figure(figsize=(10, 8))
    sns.heatmap(label_all_save, annot=False, cmap=cmap, cbar=True, xticklabels=False, yticklabels=False, vmin=-4, vmax=4)
    plt.title(f"Label's heatmap")
    plt.show()

    plt.figure(figsize=(10, 8))
    sns.heatmap(predict_all_save, annot=False, cmap=cmap, cbar=True, xticklabels=False, yticklabels=False, vmin=-4, vmax=4)
    plt.title(f"Prediction's heatmap")
    plt.show()

    plt.figure(figsize=(10, 8))
    sns.heatmap(error, annot=False, cmap=cmap, cbar=True, xticklabels=False, yticklabels=False, vmin=-4, vmax=4)
    plt.title(f"Absolute Error's heatmap")
    plt.show()
